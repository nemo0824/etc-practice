...
public class A_UncheckedException {
	 ...
	 * -NegativeArraySizeException : 배열의 크기를 음수로 지정할경우 발생하는 예외
	...
	public void method1() {
		...
		int num2 = sc.nextInt();
		//해결방법 1. 조건문으로 처리 (애초에 예외처리가 발생되지 않게끔 if문으로 조건검사하기) => 예외처리 x
//		if(num2 == 0) {
...
//		System.out.println("프로그램 종료.");
		...
		try {
			...
			System.out.println("0으로 나눌수 없습니다.");
			e.printStackTrace(); // 현재발생한 오류 추적할수있는 로그 정보 제공
		}
		System.out.println("프로그램 종료.");
	...
	public void method3() {
		//ArrayIndexOutOfBoundsException : 배열의 부적절한 인덱스로접근시 발생하는 예외
...
//			int [] arr = new int[Math.abs(size)]; //방법2 항상 양읮 어수값이 들어갈수 있도록 abs메서드 사용
//			}
		...
		System.out.println("배열의 크기를 음수로 지정할수 없습니다.");
			...
			System.out.println("예외가 발생하긴 해네요. 근데 배열의 크기를 음수로 입력했거나, 부적절한\" + \"인덱스로 접근한것 같습니다");
			
		}//다중 catch에서는 블럭의 순서가 중요함. => 범위가 작은 자식타입의 예외 클래스를 먼저기술해야함
	}
		/*
		 * RuntimeException 관련된 예외는
		 * - 조건문으로 해결 가능 => 예외 자체가 발생 안되게끔 개발자가 소스코드로 핸들링 가능
		 * - 예외처리 구문으로도 해결 가능 => 예외 발생했을때를 대비해서 그 때 실행할 내용을 정의해두는것
		 * 
		 * 예측이 가능한상황 => 조건문으로 해결 (권장하는 방법)
		 * 예측이 불가능한상황 => 예외처리 구문으로 해결
		 * 
