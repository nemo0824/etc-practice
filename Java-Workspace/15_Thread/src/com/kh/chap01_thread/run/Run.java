...
public class Run {
	 ...
	 * - 현재 동작(실행)하고 있는 프로그램
	 * 스레드
	 * -한개의 프로세스(provess)내에서 실제로 작업을 수행하는 소단위
	 * -모든 프로세스에는 하 ㄴ개 이상의 스레드가 존재하며 작업을 수행함(일꾼)
	 * 
	 * 멀티스레드
	 * -핸 개의 프로그램을 실행하고 그 내부적으로 여러 작업(스레드)를 처리하는 것
	 * - 멀티스레드의 장점
	 * 1)자원을 효율적으로 사용할 수 있다
	 * 2)사용자 입장에서 일처리가 빠르게 보인다.
	 * 3) 작업이 불리되어 코드가 간결해진다.
	 * 
	 * ex) 
	 * - 청소(나, 1시간) , 빨래(세탁기, 2시간) , 설거지(나, 30분), 화장실청소(나,30분)
	 * - 무조건 주체가 되는 중심이되는 스레드가 메인스레드. == 자바의 메인메소드
	...
	//항상 앞 라인의 명령문이 다 끝난 후에 실행이 되었음
		...
		//매번결과가 다르게 보일것. => 스케줄에 따라 순차적으로 번갈아가며 작업이 수행되기 때문.
	}
