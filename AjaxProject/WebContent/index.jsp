...
<body>
	...
	<p>
		...
		* 동기식/비동기식 <br>
		...
		요청을한다고해서 다른페이지로 넘어가지 않음(현재 페이지 유지)<br>
		요청을 보내놓고 그에 해당하는 응답이 돌아올때 까지 현재 페이지에서 다른 작업을 할수있음<br>
	...
	<pre>
		...
		* 부수적인 속성
		...
		- dataType   : response로 오는 데이터의 자료형설정(값을 추가안하면 스마트하게 판단함)
		 ...
		 - dataFilter : reponse데이터를 받았을때 정상적인 값을 return할 수 있도록 데이터와 데이터 타입 설정
		 - global : 기본 이벤트 사용여부 =>버퍼링같이 시작과 끝을 나타낼대, 선처리 작업시 사용함.
		 - password : 서버에 접소권한이 필요한 경우 설정
		...
		$(function(){
			...
			$("#btn1").click(function(){
				...
				$.ajax({
					...
					success : function(result){ // 매개변수에는 서블릿으로부터 전달받은 응답데이터가 담겨있따.
						console.log(result);
						$("#output1").text(result);
					},
					error : function() {
	...
	<h3>3. 서버로 데이터전송 후 , 조회된 객체를응답데이터로 받기</h3>
