 ...
 * -얕은 복사 : 배열의 주소값만 복사 
 * 
 * -깊은 복사 : 동일한 새로운 배열을 하나더 만들어서 실제 내부값을 복사
 * 
 * 
 */
	...
	//복사본 배열만 가지고 수정을 했는데도 원본 배열도 함께수정된다
	...
	public void method3() {
		//2. 새로운 배열을 생성한 후 System ㅋ르래스에서 arraycopy메서드를 이용
...
//		System.arraycopy(origin, 2, copy, 1, 3);
		// 0 0 0 0 0 0 0 0 3 4 인덱스의 범위를 벗어난 복사는 불가능함
	...
	/*
	 * 2. System.arraycopy 메소드 : 몇번 인덱스부터 몇개를 어느 위치의 인덱스에 복사할 것인지 세밀하게 지칭가능
	 * 
	 * 3. Arrays.Copyof 메소드 : 무조건 원본배열의 0번 인덱스부터 내가 제시한 갯수만큼 복사 진행됨 내가 제시한 길이가 
	 * 원본배열도 크다면 나머지는 0의 값으로 채워서 복사해줌.
	 * 
	 * 
	 * 
	 */
	 public void method5() {
		 ...
		 //복사본 배열 = 원본배열.clone();
		 int[] copy = origin.clone(); //인덱스 직접 지정x, 복사할 갯수 지정 x
