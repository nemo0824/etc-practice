...
<body>
   ...
   <p>
      ...
      => 응답페이지가 돌아와야 볼 수 있으며 응답페이지가 호출되기전까지 아무 요청도 하지못함<br>
      비동기식 요청을 보내기 위해서는 Ajax라는 기술이 필요함
      <br><br>
      
      * 동기식/비동기식 <br>
      - 동기식 : 요청 처리 후 그에 해당하는 응답페이지가 돌아와야만 그 다음 작업 가능함<br>
      만약 서버에서 호출된 결과까지의 시간이 지연되면 계속 기다려야함(흰화면으로 보이게됨)
      전체 페이지가 리로드됨(새로고침 즉, 페이지가 기본적으로 한번 깜박거림)<br><br>
      
      -비동기식 : 현재 페이지를 그대로 유지하면서 중간중간 추가적인 요청을 보내줄 수 있음<br>
      요청을 한다고 해서 다른 페이지로 넘어가지 않음(현재 페이지 유지)<br>
      요청을 보내놓고 그에 해당하는 응답이 돌아올때까지 현재 페이지에서 다른 작업을 할 수 있다.
   ...
   <pre>
      ...
      * 주요속성
      ...
      - data : 요청시 전달할 값 //xhr.send("id=user01");
      - success : ajax통신 성공시 실행할 함수를 정의(이벤트핸들러)
   ...
   <script>
      $(function(){
         ...
         $("#btn1").click(function(){
            ...
            // 비동기식통신 : 페이지전환이 되지 않는다.
            $.ajax({
               url : '<%=contextPath%>/jqAjax1.do',
               type : 'get',
               data : {input : $("#input1").val()}, // 요청 <input type="text" id="input1">
               success : function(result){ //매개변수에는 서블릿으로부터 전달받은 응답데이터 담겨있다.
                  console.log(result);
                  $("#output1").text(result);
               },
               error : function(){
