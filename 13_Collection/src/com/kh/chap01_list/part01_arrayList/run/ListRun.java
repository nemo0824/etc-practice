...
public class ListRun {
	...
	public static void main(String[] args) {
		 ...
		 * 
		 * 자료구조?
		 * 방대한 데이터를 효율적으로(구조적으로)다룰때 필요한 개념
		 * 
		 * 프레임워크
		 ...
		 *  => 한번 지정된 크기는 변경 불가
		 *  	새로운 값을 추가하고자 할때 크기가 오버될 경우 새로운크기의 배열을 만들고 기존의 내용들을 복사해주는
		 *  	코드를 직접 구현해야함
		 ...
		 *  	코드가 이미 정의 되어있음.
		 *  2. 중간에 값을 추가하거나 삭제하는 경우 값을 땡겨주는 코드가 이미 메소드로 정의도어 있음.
		 ...
		 * 
		 * 컬렉션의 3가지 분류
		 * List계열 => 담고자 하는 값(value)만 저장/값 저장 시 순서 유지됨(index의개념이 있음) /중복값 허용
		 * 			ex)ArrayList, Vector, LinkedList => ArrayList가 제일 많이 쓰임(90%이상)
		 * Set 계열 => 하고자 하는 값(value)만 저장 / 값 저장 시 순서 유지 x(index개념이 없음) 중복값 허용 x
		 * 			ex) Hashset, TreeSet
		 * 			
		 * Map 계열 => 키(key) + 담고자하는 값(value)세트로 저장 / 값 저장시 순서유지 x(index 개념없음)
		 * 			/key 중복 허용 x
		 ...
		 * 해당 컬렉션에 다양한 타입의 데이터 값들이 담길수 있음. (Default E == Object)
		 * 
		 * 별도의 제네릭 설정으로 <Music>으로 하게 되면 (E == Music)
		 * 
		 * 제네릭을 설정 하는 이유 
		 * 1. 명시한 타입의 객체만 저장 가능하도록 타입의 제한을 두기 위해서 
		 * 2. 컬렉션 저장된 객체를 꺼내서 사용할 때 매번 형 변환 하는 절차를 없애기 위해서.
		 *
		 *  
		 * 
		 * 
...
//		list.add("흐흐");
		...
		list.add(1, new Music("바다의왕자", "박명수"));
		//중간에 값 추가시 알아서 기존의 값들을 뒤로 땡겨주는 작업이 내부적으로 진행됨.
		
		System.out.println(list);
		
		//3. set(int index, E e) : 리스트에 원하는 위치에 인덱스 값을 제시하면 전달하는 e값으로 변경해주는 메소드
		...
		System.out.println("리스트의 담긴 데이터 수 : " +list.size());
...
//		System.out.println(list.get(0));
		...
		//7.subList(int index1, int index2) : 해당 리스트로부터 index1에서 index2까지의 데이터값들을 추출해서 새로운 list로 반환시켜주는 메소드
		
		List<Music> sub =list.subList(0, 2); // 0<= 인덱스 범위 < 2
		System.out.println(sub);
		
		System.out.println("================================================");
		//8. addAll(collection c) : 해당 리스트에 있는 다른 컬렉션에 있는 데이터들을 통채로 추가해주는 메소드
		list.addAll(sub);
		System.out.println(list);
		
		//9. isEmpty() : 해당리스트가 비어있는지 묻는 메소드(비어있으면true/비어있지않다면 false 반환)
		System.out.println("리스트가 비어있습니까?" + list.isEmpty());
		
		//10. clear() : 해당리스트를 비워주는 메소드
		list.clear();
		System.out.println(list);
		System.out.println("리스트가 비어있습니까?" + list.isEmpty());
