...
//			System.out.println("잘 실행됩니까?");
			...
			//1.전소오디는 파일을 처리할 작업 내용(전송되는 파일의 용량제한, 전달된 파일을 저장할경로
			//1_1. 전송파일 용량제한(int maxsize=> byte단위의 값을 기술해야함.) 저희는 10mbyte로 제한할 예졍
			...
			/*
			 * 세션 객체에서 제공하는 getRealPath메소드를 통해 알아낼 에정
			 * 다만 webContents 폴더로부터 board_upfiles폴더까지의 경로는 매개변수로 제시해줘야한다.
			 * 그리고 결론저그올 board_upfiles 폴더 내부에 파일들이 저장될 것이기 때문에 /를 마지막에 붙여줘야한다
			 * 
			 */
...
//			System.out.println(savePath);
			 ...
			 * MultipartRequest multi = new MultipartRequset(request 객체, 저장할 폴더 경로, 용량제한, 인코딩값, 
			 * 				파일명을 수정시켜주는 객체 );
			 * 
			 * 위 구문 한줄 실행만으로 넘어온 첨부파일들이 해당폴더에 업로드됨
			 * 그리고 사용자가 올린 파일명은 그대로 해당 폴더에 업로드 하지 않는게 관례
			 * -같은 파일명이 있을경우 덮어씌워지거나 에러가 날수 있고, 한글/특수문자/띄어쓰기가 포함된 파일명일 경우 사용하는
			 ...
			 *  -기본적으로 동일한 파일명이 존재할 경우 파일명 뒤에 카운팅된 숫자를 붙ㅇ서 유니크한 파일명을 만듦
			 *  
			...
			// - 카테고리번호, 제목 , 내용, 작성자 번호를 뽑아서 Board에 Insert
			// - 넘어온 첨부파일이 있따면, 원본명, 수정명, 폴더경로를 뽑아서 Attachment테이블에 Insert
			...
			// 첨부파일이 있을경우  원본명을 반환해주고, 없을경우 null로 반환해줌
			...
			if(result > 0) {//성공적으로 작성 => 최신글 목록으로 이동
				...
				request.setAttribute("errorMsg", "게시글 작서 실패");
			...
			request.setAttribute("errorMsg", "전송방법이 잘못되었씁니다");
			request.getRequestDispatcher("views/common/errorPage.jsp").forward(request, response);
		}
