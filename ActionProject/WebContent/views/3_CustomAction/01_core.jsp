...
- 기본값 : value에 출력하고자하는 값이 없을경ㅇ우 기본값으로 출력할 내용물을 기술(생략가능) 
...
- 각 조건들을 c:choose 태그의하위요소로 c:when, c:otherwise를 통해 작성
...
<c:choose>
	...
	<c:otherwise>
		<b>num1은 10 혹은 2와 일치하지않습니다</b> <br>
	</c:otherwise>
</c:choose>
...
<h3>4.반복문</h3>
<pre>
for loop문 - (&lt;c:foreEach var="변수명" begin="초기값" end="끝값" step="증가값(생략가능)"&gt;)
...
&lt;c:url var="변수명" value="요청할url"&gt;
	...
	&lt;c:param name="키값" value="벨류값" /&gt;
&lt;/c:url&gt;
-url경로를 생성하고, 쿼리스트링을 정의할수 있는 태그
-넘겨줘야할 쿼리스트링이 길경우 사용하면 편리하고,
-특정조건에 따라 파라미터값들을 다르게 보내야할때 자주 사용됨
